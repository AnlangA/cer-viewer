...
cer-viewer 是一个使用 Rust 编写的现代化 X.509 证书和 CSR（证书签名请求）查看器，基于 egui/eframe 构建 GUI，同时提供功能完整的 CLI 模式。

### 核心能力

- **多格式支持**: PEM/DER 证书、CSR、PKCS#12、PKCS#7/CMS
- **证书链处理**: 自动构建、AIA 补全、签名验证
- **双模式**: GUI（图形界面）和 CLI（命令行）
- **安全特性**: 敏感数据检测、内存擦除、安全警告
- **生成功能**: 自签名证书和 CSR 生成
...
- **最低 Rust 版本**: 1.80
- **技术栈**:
  - GUI: egui 0.33.3 / eframe 0.33.3
  - 证书解析: x509-parser 0.18.1
  - 证书生成: rcgen 0.14.7
  - CLI: clap 4.5
  - 网络: reqwest 0.12
  - 错误处理: thiserror 2.0
...
```
文件加载 → 格式检测 → PEM/DER 解析 → 文档封装 → UI/CLI 渲染
    ↓         ↓           ↓           ↓           ↓
  rfd/CLI  detect_format  x509-parser  Document  tab_bar/details_view
```
