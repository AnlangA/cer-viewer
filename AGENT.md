...
- ✅ JSON 输出格式
- ✅ 字段过滤
- ✅ 子命令支持（chain, extract, verify, convert）
- ✅ PEM ↔ DER 格式转换（`convert` 子命令）
...
### 阶段 9: CLI 模式 ✅
- clap 集成
- 命令实现（chain, extract, verify, convert）
- JSON 输出
- PEM ↔ DER 格式转换（`convert` 子命令）

### 阶段 10: 证书链验证 ✅
- 实现 `ChainValidator::validate()` 真正的验证逻辑
- 颁发者/主题 DN 匹配检查
- CA 基本约束检查（非叶证书必须有 CA 标志）
- 信任根检查（自签名或在信任存储中）
- 完整的单元测试覆盖

### 阶段 11: 格式转换 ✅
- `convert` CLI 子命令（PEM → DER、DER → PEM）
- 自动格式检测（无需指定输入格式）
- `formats::x509` 模块整洁化（正确的 `pub use` 重导出）
...
# 格式转换
cargo run -- convert file.pem file.der --to der
cargo run -- convert file.der file.pem --to pem
```
